...
- 目标是把通用 worktree 斜杠命令安装到 `~/.codex/`，供任意 Git 项目复用。
- 当前目录应只包含通用工具，不放具体业务项目代码。

## 目录结构

- `install.py`：安装或更新 HOME 级 Codex worktree 命令。
- `prompts/`：Codex 自定义斜杠命令模板，安装到 `~/.codex/prompts/`。
- `scripts/`：命令执行脚本，安装到 `~/.codex/scripts/`。
- `README.md`：默认中文说明。
- `README.en.md`：英文说明。
- `LICENSE`：MIT 许可证。
...
- 不再提供 `/worktree-install` 斜杠命令；安装和升级统一由 `install.py` 完成。
- 修改命令行为时，优先更新 `scripts/worktree-task.py`，prompt 只保留简洁的调用说明和安全约束。
...
- 本机任务状态必须按仓库隔离保存在 `~/.codex/worktree-state/`，不得写入业务项目仓库。
