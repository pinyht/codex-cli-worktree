...
- 常用命令：
  ...
  - `$worktree-push-sql 任务名 SQL文件...`：把任务 worktree 中新增的 SQL 文件拿到主项目目录并删除任务副本，自动 add、commit、push、sync --all，再切换到该任务预览。
...
- `$worktree-push-sql` 只支持 `.sql` 文件，一次可指定多个；执行前主项目目录必须干净或等于已记录的 switch 预览，执行后会自动提交、推送、同步所有任务并切换到该任务预览；提交信息格式为 `add SQL文件 SQL文件`。
