...
- 合并任务时，先把主项目目录恢复干净，再把任务改动三方合并回主项目目录，但不自动 commit；遇到冲突会保留冲突状态，并由 Codex 基于冲突上下文和本次合并完整改动范围给出 AI 语义级整体修复方案。
...
- `$worktree-merge` 会在合并任务改动前恢复并清理主项目调试槽，使用 Git 三方合并写入主项目目录，不自动 commit；遇到冲突时脚本保留冲突状态并输出基础分析，随后 Codex 读取冲突文件、三方版本和本次合并完整改动范围，检索相关路由、菜单、API、权限、配置、schema、测试和文档，给出 AI 语义级整体修复方案供你选择，确认后再修改文件解决冲突。
